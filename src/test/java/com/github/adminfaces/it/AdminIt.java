...
public class AdminIt {
    ...
    public void shouldCountCars() {
        assertEquals(guarantorFacade.count().intValue(), 4);
    }

    @Test
    @UsingDataSet("cars.yml")
    public void shouldFindCarById() {
        Car car = guarantorFacade.findById(1);
    ...
    public void shouldFindCarByExample() {
        ...
        List<Car> cars = guarantorFacade.example(carExample,Car_.model).getResultList();
    ...
    public void shouldNotInsertCarWithoutName() {
        long countBefore = guarantorFacade.count();
        assertEquals(countBefore, 0);
        Car newCar = new Car().model("My Car").price(1d);
        try {
            guarantorFacade.insert(newCar);
    ...
    public void shouldInsertCar() {
        ...
        guarantorFacade.insert(newCar);
        assertEquals(countBefore + 1, guarantorFacade.count().intValue());
    }
    ...
    public void shouldRemoveCar() {
        assertEquals(guarantorFacade.count(guarantorFacade.criteria().eq(Car_.id,1)).intValue(),1);
        Car car = guarantorFacade.findById(1);
        assertNotNull(car);
        guarantorFacade.remove(car);
        assertEquals(guarantorFacade.count(guarantorFacade.criteria().eq(Car_.id,1)).intValue(),0);
    }

    @Test
    @UsingDataSet("cars.yml")
    public void shouldListCarsModel() {
        List<Car> cars = guarantorFacade.listByModel("%porche%");
    ...
    public void shouldListCarsByPrice() {
        List<Car> cars = guarantorFacade.criteria()
    ...
    public void shouldGetCarModels() {
        List<String> models = guarantorFacade.getModels("po");
